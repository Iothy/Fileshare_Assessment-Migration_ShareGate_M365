<#
.SYNOPSIS
    Dashboard décisionnel multi-FileShare pour l'assessment Carambar.

.DESCRIPTION
    Ce script lit les fichiers CSV générés par les autres scripts d'assessment
    et produit un dashboard HTML exécutif présentable pour le client Carambar.
...
.NOTES
    ...
    v3.3 (2026-05-10)
      ...
      - Correction 3 : Detection groupes AD elargie.
        ...
        Enterprise/Schema Admins, _G_/_U_ entre underscores (nomenclature Carambar).
    ...
    v3.2 (2026-05-05)
      - Nettoyage perimetre client :
        ...
        5. Blocklist extensions : l'export consomme uniquement le CSV produit par Get-BlockedExtensions.ps1
           (pas de refiltrage interne), conforme a la liste officielle Carambar.
...
Write-Log "=== Génération du dashboard décisionnel Carambar v3.4 ===" "INFO"
...
function Build-SectionSummary {
    ...
    $volStr        = Format-VolumeReadable $VolumeGB
    $nomFsDisplay  = if ([string]::IsNullOrWhiteSpace($NomFS)) { "Carambar" } else { $NomFS }
    ...
    if ($DataPermissions.Count -gt 0) {
        ...
        foreach ($ig in ($DataPermissions | Group-Object $idCol)) {
            ...
            if ($idRef -match '^S-\d+-') {
                ...
                # v3.3 — Heuristique elargie pour groupes AD Carambar (ordre de priorite)
                ...
                if ($idName2 -in $wellKnownGroups) {
                    $idType = "Group"
                # 2. Domaine SHVE avec nomenclature USR_BTQ (groupes Carambar)
...
<div class="report-footer">
  ...
  <p>Carambar &times; Avanade &mdash; Propulse par ShareGate</p>
</div>
"@
}
...
function Build-FullReport {
    param(
        [string]$Title          = "Dashboard Assessment — Carambar",
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Index Assessment &mdash; Carambar DSI</title>
  <style>$css</style>
</head>
<body>
<div class="page-header">
  <h1>&#x1F5C2; Index Assessment &mdash; Carambar DSI</h1>
...
$unmappedSection
<div class="report-footer">
  <p>Carambar &times; Avanade &mdash; Propulse par ShareGate | v3.4 &mdash; ReportOutputPath + permissions NTFS systematiques</p>
...
if ($fsMapping.Count -gt 0 -and -not $SplitByLevel1) {
    ...
    foreach ($fs in $fsMapping) {
        ...
        $rptFile = Join-Path $reportOutputPath "Rapport_Assessment_Carambar_${safeFsName}_$timestamp.html"
    ...
    # ---- Rapport GLOBAL ----
    $globalFile = Join-Path $reportOutputPath "Rapport_Assessment_Carambar_GLOBAL_$timestamp.html"
    Write-Log "  -> Rapport global (confidentiel DSI)..." "INFO"
    try {
    $htmlGlobal = Build-FullReport `
        -Title         "Rapport Assessment Global Carambar (CONFIDENTIEL DSI)" `
    ...
    # ---- Index DSI ----
    $indexFile = Join-Path $reportOutputPath "Index_Assessment_Carambar_$timestamp.html"
    ...
    # ================================================================
    $rapportFile = Join-Path $reportOutputPath "Rapport_Assessment_Carambar_$timestamp.html"
    Write-Log "Mode standard — generation du rapport global..." "INFO"

    try {
    $html = Build-FullReport `
        -Title         "Rapport Assessment Carambar" `
